#!/bin/sh
# nicenumber -- Given a number, shows it in comma-sperated form.
# Expects DO and TD to be instantiated. Instantiates nicenum
# or, if a second arg is specified, the output is echoed to stdout.

nicenumber()
{
	# Note that we assume that '.' is the decimal seperator in 
	# the INPUT value to this script. The decimal seperator in the output value is
...
while [ $thousands -gt 999 ]; do
	...
	result="${TD:=","}${remainder}${result}"	#builds left to right
done
...
DD="." # decimal point delimiter, to seperate integer and fractional values
