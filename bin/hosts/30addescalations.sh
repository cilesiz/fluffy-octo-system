...
if [ $# != 4 ]; then
        echo -e "\n./$SCRIPT requires exactly 4 args"
...
# to do
# show_contacts.sh | grep -v page select menu
...
case "$hostlevel" in
1) cat /usr/local/nagios/DNA/skel/hosts/30addescalations/level1_host_escalation.skel /usr/local/nagios/DNA/skel/hosts/30addescalations/level3_service_escalation.skel  /usr/local/nagios/DNA/skel/hosts/30addescalations/level5_service_escalation.skel  /usr/local/nagios/DNA/skel/hosts/30addescalations/level3_cg.skel | sed -e "s/HOSTGROUP/$hostgroup/g" -e "s/TOWNER/$owner/g" -e "s/SYSENG/$syseng/g"  >> /usr/local/nagios/DNA/tmp/$hostgroup.escalations.cfg ;;
2) cat /usr/local/nagios/DNA/skel/hosts/30addescalations/level2_host_escalation.skel /usr/local/nagios/DNA/skel/hosts/30addescalations/level4_service_escalation.skel /usr/local/nagios/DNA/skel/hosts/30addescalations/level6_service_escalation.skel  /usr/local/nagios/DNA/skel/hosts/30addescalations/level4_cg.skel | sed -e "s/HOSTGROUP/$hostgroup/g" -e "s/TOWNER/$owner/g" -e "s/SYSENG/$syseng/g" >> /usr/local/nagios/DNA/tmp/$hostgroup.escalations.cfg ;;
*) printf "%s\n" "please choose either 1 or 2 for a host level"; exit 1 ;;
esac
	echo
	printf "%s\n" "OK - /usr/local/nagios/DNA/tmp/$hostgroup.escalations.cfg created succesfully"
	printf "%s\n" "please review the file and then issue the following command:"
	printf "%s\n" "cp /usr/local/nagios/DNA/tmp/$hostgroup.escalations.cfg /usr/local/nagios/etc/escalations/"
	echo
	# no longer running, remove stale lock file
                rm -f "$lockfile"
	exit 0
