...
# set the skel dir
declare -rx SKEL="/usr/local/nagios/DNA/skel/hosts/10addhost"
...
declare -rx CONTACTS="/usr/local/nagios/etc/contacts/contacts.cfg"
...
# TODO: add some logic to lookup current $TOWNER's via grep
	grep -qo "$TOWNER" $CONTACTS
if [ $? -eq 1 ]; then
	echo -e "\n-ERROR- The email address the TechOwner $TOWNER doesnt exist in $CONTACTS\n"
	exit 1
fi
